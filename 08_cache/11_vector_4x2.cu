...
#pragma unroll
    for (int k = 0; k < 8; k++) {
      for (int i = 0; i < 2; ++i) {
	for (int j = 0; j < 4; ++j) {
	  fragment_a[i * 4 + j] = block_a[k][offset_m + i * 16 + j];
	  fragment_b[i * 4 + j] = block_b[k][offset_n + i * 32 + j];
	}
      }
      for (int m = 0; m < 8; ++m) {
	for (int n = 0; n < 8; ++n) {
  ...
  for (int i_n = 0; i_n < 8; ++i_n) {
    for (int i_m = 0; i_m < 8; i_m += 4) {
      int c_n = offset_b_n + offset_n + (i_n / 4) * 32 + (i_n % 4);
      int c_m = offset_a_m + offset_m + (i_m / 4) * 16 + (i_m % 4);
      for (int i = 0; i < 4; ++i) {
	if (c_n < dim_n && (c_m + i) < dim_m) {
	  d_c[c_n * dim_m + c_m + i] = block_c[i_m + i][i_n];
	}
      }
    }
  }
